...
def build_keypoints(count=16, width=0.25, x_offset=0.3, z_offset=0.25):
    ...
    points = get_lemniscate_keypoint(t, width)

    x_coord = np.ones_like(t)*x_offset

    coords = np.stack([x_coord, points[0], points[1] + z_offset], axis=-1)
...
def ik_track(model, data, site_name, target_pos,
    ...
    for i in range(max_iters):
        ...
        if np.linalg.norm(err_pos, ord=2) < 1e-3:
            break
        ...
        weighted_err = np.hstack([pos_gain * err_pos, np.zeros(3)])
        print((J @ J.T + damping * np.eye(6)).shape)
        print(weighted_err.shape)
